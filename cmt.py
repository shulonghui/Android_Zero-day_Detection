...
#PyTorch 中实现 Swish 激活函数的代码,在某些情况下可以比 ReLU 激活函数更好地缓解梯度消失问题。
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        # 修改：引入非对称卷积核大小和分组数来解决对称性限制
        asym_kernel_size = 5
        asym_groups = hidden_features // 2
        if hidden_features % asym_groups != 0:
            raise ValueError("in_channels must be divisible by groups")


        self.proj = nn.Conv2d(hidden_features, hidden_features, asym_kernel_size, 1, asym_kernel_size // 2, groups=asym_groups)
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # 使用深度可分离卷积替换原来的卷积层
        self.proj_depthwise = nn.Conv2d(dim, dim, 3, 1, 1, groups=dim)
        self.proj_pointwise = nn.Conv2d(dim, dim, 1)

    def forward(self, x, H, W, relative_pos):
        B, N, C = x.shape
        cnn_feat = x.permute(0, 2, 1).reshape(B, C, H, W)
        x = self.proj_depthwise(cnn_feat)
        x = self.proj_pointwise(x)
        x = x + cnn_feat
        x = x.flatten(2).permute(0, 2, 1)
        x = x + self.drop_path(self.attn(self.norm1(x), H, W, relative_pos))
        x = x + self.drop_path(self.mlp(self.norm2(x), H, W))
...
#def _create_cmt_model(pretrained=True, distilled=False, **kwargs):
    default_cfg = _cfg()
...
#def cmt_xs(pretrained=True, **kwargs):
    """ 
...
#def cmt_s(pretrained=True, **kwargs):
    """ 
    CMT-Small
    """
    model_kwargs = dict(
...
#def cmt_b(pretrained=True, **kwargs):
    """ 
    CMT-Base
    """
    model_kwargs = dict(
